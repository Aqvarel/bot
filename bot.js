...
function matches(rule, msg) {
  ...
  return hit(m.from_includes, from) && hit(m.subject_includes, subject) && hit(m.body_includes, body);
}

async function checkOnce() {
  const cfg = readJson(RULES_FILE);
  const state = readJson(STATE_FILE, { processed: [], lastReplyTo: {} });
  const me = state.me || (state.me = (await graph('GET', '/me')).userPrincipalName.toLowerCase());
  ...
  for (const msg of res.value) {
    ...
    const full = await graph('GET', `/me/messages/${msg.id}?$select=body,subject,from`);
    ...
    if (looksLikePaymentRequest(text)) {
      ...
      try {
        ...
        await notify(`⚠️ Заявка от ${from} НЕ записана в базу: ${e.message.slice(0, 300)}`);
      }
      await graph('POST', `/me/messages/${msg.id}/reply`, { comment: cfg.payment_request_reply });
    ...
    state.lastReplyTo[from] = Date.now();
    console.log(`ответил (${rule ? rule.name : 'общий шаблон'}):`, from, '|', msg.subject);
    await notify(`📧 Автоответ для ${from}\nТема: ${msg.subject || '(без темы)'}\nПравило: ${rule ? rule.name : 'общий шаблон'}`);
  }
...
async function main() {
  ...
  while (true) {
    try {
      ...
      if (e.message.includes('token')) await notify('⚠️ Почтовый бот: проблема с токеном, нужен повторный вход.');
    }
    if (once) break;
