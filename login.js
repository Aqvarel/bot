...
const {
  CLIENT_ID, TENANT, SCOPE, DEVICE_FILE,
  notify, readJson, writeJson, saveToken, graph,
} = require('./lib');
...
async function poll() {
  ...
  if (!dev) throw new Error('device.json не найден — сначала node login.js request');
  ...
  while (Date.now() < deadline) {
    ...
    if (res.ok) {
      ...
      await notify(`✅ Почтовый бот: вход в ${who} выполнен, токены сохранены.`);
      return;
    }
  ...
  console.log('EXPIRED: код истёк, нужно запросить новый (node login.js request)');
  process.exit(2);
}
